...
export class PushService {
  ...
  async subscribe(params: { endpoint: string; keys: { p256dh: string; auth: string } }): Promise<void> {
    const existing = await this.em.findOne(PushSubscription, { endpoint: params.endpoint });
    if (existing) return;
  ...
  async unsubscribe(endpoint: string): Promise<void> {
    const subscription = await this.em.findOne(PushSubscription, { endpoint });
    if (subscription) await this.em.removeAndFlush(subscription);
  }

  async sendToAll(payload: string): Promise<void> {
